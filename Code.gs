/**
 * PULSE BROADCAST // PURE LIVE AUDIO ENGINE
...
function handleRequest(e, method) {
  ...
  try {
    ...
    switch (action) {
      ...
      case "uploadMedia":
        responseData = handleAudioUpload(params);
        break;
      ...
      case "streamAudio":
      case "streamMedia":
        responseData = handleStreamAudio(params.fileId);
        break;
      default:
...
function handleAudioUpload(params) {
  ...
  const mime = metadata.mimeType || "audio/webm";
  ...
  const contentId = "aud_" + Utilities.getUuid().substring(0, 12);
  ...
  sheet.appendRow([
    contentId,
    fileId,
    metadata.title || "Untitled Audio Transmission",
    metadata.fileName,
    "audio",
    mime,
  ...
  return { success: true, data: { contentId: contentId, fileId: fileId, mediaType: "audio" } };
}
...
function reconcileDriveFolderWithSheet() {
  ...
  // 1. Gather all active audio files in folder
  ...
  // 2. Scan sheet rows bottom-to-top to prune removed files
  const data = sheet.getDataRange().getValues();
  ...
  // 3. Register only audio files
  activeFilesInFolder.forEach((file, fileId) => {
    if (!existingSheetFileIds.has(fileId)) {
      const mime = file.getMimeType();
      const isAudio = mime.startsWith("audio/") || file.getName().match(/\.(mp3|wav|ogg|m4a|flac|webm|aac)$/i);

      if (isAudio) {
        file.setSharing(DriveApp.Access.ANYONE_WITH_LINK, DriveApp.Permission.VIEW);

        sheet.appendRow([
          "aud_" + Utilities.getUuid().substring(0, 10),
          fileId,
...
function handleGetLiveManifest(listenerId, incomingEtag) {
  ...
  for (let i = 1; i < data.length; i++) {
    const mType = String(data[i][4]).toLowerCase();
    // Only return audio items
...
function handleStreamAudio(fileId) {
